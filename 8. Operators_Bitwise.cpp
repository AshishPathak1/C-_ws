...
void one_compliment_demo()
{
	...
	*/
	...
	std::cout << "Integer data3 : " << idata3 << std::endl <<
		"\t(Binary) " << std::bitset<32>(idata3) << std::endl;

	int idata4 = ~idata3;

	std::cout << "Integer data4 : " << idata4 << std::endl <<
		"\t(Binary) " << std::bitset<32>(idata4) << std::endl;

	int8_t i8 = 100;
	std::cout << "Integer i8 : " << static_cast<int>(i8) << std::endl <<
		"\t(Binary) " << std::bitset<8>(i8) << std::endl;
}
...
void right_shift_demo()
{
	...
	i1n = INT_MAX;//INT_MAX;    std::numeric_limits<int>::max()
	ibitpos = 5;
	std::cout << "Integer i1n : " << i1n << std::endl <<
		"\t(Binary) " << std::bitset<32>(i1n) << std::endl;
	/*When the number is +ve the blank spaces introduced on right side are padded with zero*/
	iresult = i1n >> ibitpos;
	std::cout << "Integer result : " << iresult << std::endl <<
	...
	std::cout << "Integer i1n : " << i1n << std::endl <<
		"\t(Binary) " << std::bitset<32>(i1n) << std::endl;
	/*When the number is -ve the blank spaces introduced on right side are padded with One*/
	iresult = i1n >> ibitpos;
	std::cout << "Integer result : " << iresult << std::endl <<
		"\t(Binary) " << std::bitset<32>(iresult) << std::endl;
}
...
void bit_and_turn_off() {
	...
	int mask{ static_cast<int>(powf(2.0f, (static_cast<float>(ibitpos - 1)))) };
	mask = ~mask;
...
/*  Check if the bit is on or off
	...
	if ans == mask  then the bit is on otherwise the bit is off
...
void bit_and_check_status() {
	...
	int iresult = i1n & static_cast<int>(powf(2.0f, static_cast<float>(ibitpos - 1)));
...
/* Toggle bit on or off
	...
	XOR the bit with 1 to toggle its state
*/
...
void storing_date() {
	uint16_t date = (512 * (2024 - 1980) + 32 * 1 + 9);
	//Date:          22569 Binary : 0101100000101001
